...
def get_n_steps(training_step, total_steps):
    ...
    Si le modèle chargé est déjà bien entraîné, le curriculum commencera
    directement en phase avancée selon total_steps et training_step.
    """
...
if __name__ == '__main__':
    ...
    print(f'Replay buffer loaded: {len(actor.replay_buffer.observations)} transitions.')
    ...
    with open(os.path.join(replay_path_to_dir, agent_name + '_collected_rewards.pkl'), 'rb') as f:
        collected_rewards = pickle.load(f)

    # ======================================================================
    # Warm-up si le buffer chargé est insuffisant
    # ======================================================================
    if len(actor.replay_buffer.observations) < buffer_min_length:
        print('=' * 60)
        print('WARM-UP PHASE (buffer insuffisant après chargement)')
        ...
        print(f'Buffer suffisant ({len(actor.replay_buffer.observations)} transitions) — warm-up ignoré.\n')
